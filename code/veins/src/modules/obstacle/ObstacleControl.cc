...
void ObstacleControl::add(Obstacle obstacle) {
	...
	//if (annotations)
	  //  o->visualRepresentation = annotations->drawPolygon(o->getShape(), "blue", annotationGroup);
	...
	for (size_t col = fromCol; col <= toCol; ++col) {
		...
		for (size_t row = fromRow; row <= toRow; ++row) {
			...
			for (ObstacleGridCell::const_iterator k = cell.begin(); k != cell.end(); ++k) {
				...
				// draw a "hit!" bubble
				if (annotations && (factor != factorOld))
				    if(st==1){
				    annotations->drawBubble(o->getBboxP1(), "Reroute the server");
				    }
