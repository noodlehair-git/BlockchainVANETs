...
void BaseWaveApplLayer::initialize(int stage) {
	...
	if (stage==0) {
		...
		sendBeaconEvt = new cMessage("LTE_up_down_data evt", SEND_BEACON_EVT);

		//simulate asynchronous channel access
		double offSet = dblrand() * (par("data_time_interval").doubleValue()/2);
...
void BaseWaveApplLayer::handleLowerMsg(cMessage* msg) {
	...
	if (std::string(wsm->getName()) == "LTE_up_down_data") {
		onBeacon(wsm);
	}
...
void BaseWaveApplLayer::handleSelfMsg(cMessage* msg) {
	switch (msg->getKind()) {
		...
		{
		    if(simTime() >20 && simTime() <25)
		     {
			sendWSM(prepareWSM("messageEnc", beaconLengthBits, type_CCH, beaconPriority, 0, -1));
			scheduleAt(simTime() + par("data_time_interval").doubleValue(), sendBeaconEvt);
		     }
		    else if(simTime() >2 && simTime() <20)
		     {
		         sendWSM(prepareWSM("DSAdata", beaconLengthBits, type_CCH, beaconPriority, 0, -1));
		                     scheduleAt(simTime() + par("data_time_interval").doubleValue(), sendBeaconEvt);
		     }else
		     {
		         sendWSM(prepareWSM("BlockchainMsgData", beaconLengthBits, type_CCH, beaconPriority, 0, -1));
		                  scheduleAt(simTime() + par("data_time_interval").doubleValue(), sendBeaconEvt);
		     }
